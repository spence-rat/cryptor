<#
 ...
 Version: 1.1
 Description: The Crypt-It function provide the ability to encrypt/decrypt data 
and files using AES password based and public/private key encryption.
 License: BSD 3-Clause
 Required Dependencies: None
 Optional Dependencies: None
 TODO
 Add function: gen files, enc files with aes, encrypt aes with public, decrypt 
aes with private, decrypt with aes
#>
Function Crypt-It 
{
<#
 .SYNOPSIS
 The Crypt-It function provide the ability to encrypt/decrypt data and 
files using AES password based and public/private key encryption. It's a basic helper 
function.
 .PARAMETER Target
 ...
 .PARAMETER CertPath
 The cert path is used to define the path to the target folder to store or 
recover the key files.
...
The data string to be encrypted when the data target is selected.
 ...
 .EXAMPLE
 PS C:\> Get-SQLQuery -Verbose -Instance "SQLSERVER1.domain.com\SQLExpress" 
-Query "Select @@version" -Threads 15
 .EXAMPLE
 PS C:\> Get-SQLQuery -Verbose -Instance "SQLSERVER1.domain.com,1433" -
Query "Select @@version" -Threads 15
 .EXAMPLE
 PS C:\> Get-SQLInstanceDomain | Get-SQLQuery -Verbose -Query "Select 
@@version" -Threads 15
 .EXAMPLE
 ENCRYPT/DECRYPT STRING
 PS C:\> Crypt-It -Target Data -Data "This is my plaintext" -AesPassword 
"MyPassword" -EncMode Encrypt -Verbose
 PS C:\> Crypt-It -Target Data -Data 
"EAAAAE8TEWdUV0c4sW8at4P63A4HhdAvfwJB61ALbkpKtdFvUVRm6VrMvHQtaD70nkOByg==" -
AesPassword "MyPassword" -EncMode Decrypt -Verbose
 .EXAMPLE
 ENCRYPT FILE WITH PUBLIC/PRIVATE KEYS
 PS C:\> Crypt-It -Target File -EncMode Encrypt -EncType PrivateKey -
FileExt ".enc" -FilePath "c:\temp\enc\test.txt" -CertPath "c:\temp\enc" -Verbose
 PS C:\> Crypt-It -Target File -EncMode Decrypt -EncType PrivateKey -
FileExt ".enc" -FilePath "c:\temp\enc\test.txt.enc" -CertPath "c:\temp\enc" -
FileCertPrivate "myPrivate.key" -Verbose
 .EXAMPLE
 ENCRYPT FILE WITH AES PASSWORD
 PS C:\> Crypt-It -Target File -EncMode Encrypt -EncType AESPassword -
AesPassword "testing123" -FilePath C:\temp\enc\test.txt -Verbose
 PS C:\> Crypt-It -Target File -EncMode Decrypt -EncType AESPassword -
AesPassword "testing123" -FilePath C:\temp\enc\test.txt.enc -Verbose
 .EXAMPLE
 ENCRYPT FILES RECURSIVELY WITH AES PASSWORD
 PS C:\> Crypt-It -Target Folder -EncMode Encrypt -EncType AESPassword -
AesPassword "testing123" -FilePath 'C:\temp\enc\' -FileExt ".enc" -Verbose
 PS C:\> Crypt-It -Target Folder -EncMode Decrypt -EncType AESPassword -
AesPassword "testing123" -FilePath 'C:\temp\enc\' -FileExt ".enc" -Verbose
 PS C:\> Crypt-It -Target Folder -EncMode Encrypt -EncType AESPassword -
AesPassword "testing123" -FilePath 'C:\temp\enc\' -FileExt ".enc" -FileGen -FileGenNum 
10 -Verbose
 PS C:\> Crypt-It -Target Folder -EncMode Decrypt -EncType AESPassword -
AesPassword "testing123" -FilePath 'C:\temp\enc\' -Verbose
 #>
 [CmdletBinding()]
 param
 ( 
...
[string]$Target,
 ...
 if (-not 
([System.Management.Automation.PSTypeName]'Ransomware.Encryptionfunctions').Type)
 {
...
/r:System.Runtime.InteropServices.dll /target:library /out:test.dll test.cs
 // Powershell
...
---
 // syntax
// CryptFilesRecusive(string targetFilepath, string aesPassword, 
string fileExt, string genFiles, int Filenumb, string cryptMode)
 // Run encryption
// 
CryptFilesRecusive("c:\\temp\\enc","MyPassword123",".enc","Yes",1000,"1");
 // Run decryptions
// CryptFilesRecusive("c:\\temp\\enc", "MyPassword123", ".enc", 
"No", 1000, "0");
 //----------------------------------------------------------------
---
...
GenerateKeys("c:\\temp\\public.key","c:\\temp\\private.key");
 ...
 // Encrypt(string publicKeyFileName, string 
...
nc");
 ...
 // Decrypt(string privateKeyFileName, string 
encryptedFileName, string plainFileName)
 // 
...
//----------------------------------------------------------------
---
 var enctext = EncryptStringAES("test this 
now","mypassword");
 Console.WriteLine(enctext);
 var dectext = DecryptStringAES(enctext 
,"mypassword");
...
{
 // prepend the IV
msEncrypt.Write(BitConverter.GetBytes(aesAlg.IV.Length), 
0, sizeof(int));
...
// https://www.sysadmins.lv/retired-msft-blogs/alejacma/how-togenerate-key-pairs-encrypt-and-decrypt-data-with-net-csharp.aspx
 public static void GenerateKeys(string publicKeyFileName, string 
privateKeyFileName)
 {
 // Variables
...
catch (Exception ex)
{
 ...
 Console.WriteLine("Exception generating a new key pair! More 
info:");
...
--------
 // https://www.sysadmins.lv/retired-msft-blogs/alejacma/how-togenerate-key-pairs-encrypt-and-decrypt-data-with-net-csharp.aspx
 public static void EncryptWithKey(string publicKeyFileName, string 
plainFileName, string encryptedFileName)
 {
 // Variables
CspParameters cspParams = null;
...
--------
 // https://www.sysadmins.lv/retired-msft-blogs/alejacma/how-togenerate-key-pairs-encrypt-and-decrypt-data-with-net-csharp.aspx
 public static void DecryptWithKey(string privateKeyFileName, string 
encryptedFileName, string plainFileName)
 {
 // Variables
 CspParameters cspParams = null;
...
encryptedFile.Read(encryptedBytes, 0, 
(int)encryptedFile.Length);
 // Decrypt text
...
rng.GetBytes(data);
 ...
 FileStream fsCrypt = new FileStream(inputFile + fileExt, 
FileMode.Create);
 ...
 //"What it does is repeatedly hash the user password along with the 
salt." High iteration counts.
...
http://security.stackexchange.com/questions/52665/which-is-the-best-cipher-mode-andpadding-mode-for-aes-encryption
 ...
 // write salt to the begining of the output file, so in this case can 
be random every time
 fsCrypt.Write(salt, 0, salt.Length);
 CryptoStream cs = new CryptoStream(fsCrypt, AES.CreateEncryptor(), 
CryptoStreamMode.Write);
 FileStream fsIn = new FileStream(inputFile, FileMode.Open);
 //create a buffer (1mb) so only this amount will allocate in the 
memory and not the whole file
...
fsCrypt.Close();
 ...
 //-------------------------------------------------------------------
 // https://ourcodeworld.com/articles/read/471/how-to-encrypt-and-decryptfiles-using-the-aes-encryption-algorithm-in-c-sharp
 public void FileDecrypt(string inputFile, string outputFile, string 
password)
 {
 byte[] passwordBytes = System.Text.Encoding.UTF8.GetBytes(password);
...
fsCrypt.Close();
 ...
 //-------------------------------------------------------------------
 public static void DirFileEncryptRec2(string targetDirectory, string 
aesPassword, string fileExt, string genFiles, int genFilenum, string cryptMode)
 {
 // ----------------------------------------------------
 ...
 if (genFiles == "Yes")
 {
 Console.WriteLine("Generating {0} test files in directory", 
genFilenum);
...
string path = targetDirectory + "\\file" + i + ".txt";
 ...
 {
 byte[] info = new 
System.Text.UTF8Encoding(true).GetBytes("This is some text in the file.");
...
salt." High iteration counts.
 ...
 FileStream fsCrypt = new FileStream(file + fileExt, 
FileMode.Create);
 // Write salt to the begining of the new file
fsCrypt.Write(salt, 0, salt.Length);
 // Create a cyptostream to encrypt the data written to the new 
file
CryptoStream cs = new CryptoStream(fsCrypt, AES.CreateEncryptor(), 
CryptoStreamMode.Write);
...
finally
{
 ...
 //-------------------------------------------------------------------
 public void CryptFilesRecusive(string targetFilepath, string aesPassword, 
string fileExt, string genFiles, int Filenumb, string cryptMode)
 {
...
//Console.WriteLine("Decrypting {0} to {1}", file, 
newfilePath);
...
"@ 
 ...
 Write-Verbose "ENCRYPTION OPERATION: - $FilePath to 
$FilePathWithExt"
...
[CryptIt.Encryptionfunctions]::DirFileEncryptRec2($FilePath,$AesPassword,$FileExt,$Fil
eGenEnabled,$FileGenNum,$ModeSetting); 
 ...
 { 
 Write-Verbose "ENCRYPTION OPERATION: START"
 Write-Verbose "ENCRYPTION OPERATION: GENERATING KEYS (REQUIRED FOR 
DECRYPTION)"
...
[CryptIt.Encryptionfunctions]::EncryptWithKey($PublicKeyPath,$FilePath,$FilePathWithEx
t);
 ...
 $DecFile = $FilePath + ".decrypted"
 $PrivateKeyPath = $CertPath + "\$FileCertPrivate"
 [CryptIt.Encryptionfunctions]::DecryptWithKey($PrivateKeyPath 
,$FilePath, $DecFile)
 # Remove encrypted file
[System.IO.File]::Delete($FilePath)
